...
export class TemplateReader implements ITemplateReader{
  ...
  static getTemplateConfigFile(templatePath) : Error | ITemplateConfigFile {
    ...
    try {
      ...
      return new Error(ErroMsgs.UNEXPECTED_ERR("la récupération du fichier de configuration",err))
    }
  }
  ...
  async getTemplateFiles() : Promise<Error |ITemplateFileInfo[]> {
    try {
      ...
      return new Error(ErroMsgs.UNEXPECTED_ERR("la récupération des fichiers du template",err))
    }
  }
